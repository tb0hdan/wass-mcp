...
// IsAvailable checks if the nuclei binary is available.
...
func (t *Tool) Register(srv *server.Server) error {
	...
	t.logger.Debug().Msg("nuclei tool registered")

	return nil
}

// NucleiHandler handles MCP tool requests.
func (t *Tool) NucleiHandler(ctx context.Context, _ *mcp.CallToolRequest, input Input) (*mcp.CallToolResult, any, error) {
...
func (t *Tool) formatOutput(targetURL, output string, maxLines, offset int) string {
	...
	resultText := fmt.Sprintf("nuclei output for %s:\n", targetURL)
